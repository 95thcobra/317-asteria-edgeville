...
import com.asteria.game.character.combat.strategy.DefaultMagicCombatStrategy;
import com.asteria.game.character.combat.strategy.DefaultMeleeCombatStrategy;
import com.asteria.game.character.combat.strategy.DefaultRangedCombatStrategy;
...
public final class Combat {
	...
	protected static void applyPrayerEffects(CombatSessionData data) {
		...
		if (Combat.isFullVeracs(data.getAttacker())) {
			if (Server.DEBUG && data.getAttacker().getType() == NodeType.PLAYER)
				((Player) data.getAttacker()).getMessages().sendMessage("[DEBUG]: Chance of opponents prayer cancelling hit " + "[0/" + PRAYER_ACCURACY_REDUCTION + "]");
			return;
		}
		...
		if (CombatPrayer.isActivated(player, Combat.getProtectingPrayer(data.getType()))) {
			...
			case PLAYER:
				for (CombatHit h : data.getHits()) {
					...
					if (Server.DEBUG)
						player.getMessages().sendMessage("[DEBUG]: Damage " + "reduced by opponents prayer [" + (hit - h.getHit().getDamage()) + "]");
					mod = Math.round(random.get().nextDouble() * 100.0) / 100.0;
					if (Server.DEBUG)
						player.getMessages().sendMessage("[DEBUG]: Chance " + "of opponents prayer cancelling hit [" + mod + "/" + PRAYER_ACCURACY_REDUCTION + "]");
	...
	public static CombatSpells prepareSpellCast(Npc npc) {
		...
		case 172:
		case 174:
			return random.random(new CombatSpells[] { CombatSpells.WEAKEN, CombatSpells.FIRE_STRIKE, CombatSpells.EARTH_STRIKE, CombatSpells.WATER_STRIKE });
		default:
	...
	public static Hit calculateRandomHit(CharacterNode character, CharacterNode victim, CombatType type) {
		...
		case MAGIC:
			if (Server.DEBUG && character.getType() == NodeType.PLAYER)
				((Player) character).getMessages().sendMessage("[DEBUG]: " + "Maximum hit this turn is [" + character.getCurrentlyCasting().maximumHit() + "].");
	...
	public static boolean isAccurate(CharacterNode attacker, CharacterNode victim, CombatType type) {
		...
		if (attacker.getType() == NodeType.PLAYER && Server.DEBUG) {
			((Player) attacker).getMessages().sendMessage("[DEBUG]: Your roll " + "[" + (Math.round(attackCalc * 1000.0) / 1000.0) + "] : " + "Victim's roll [" + (Math.round(defenceCalc * 1000.0) / 1000.0) + "] : Chance to hit [" + (100 * Math.round(hitSucceed * 1000.0) / 1000.0) + "%]");
		}
	...
	public static boolean checkAttackDistance(CombatBuilder builder) {
		...
		/*CharacterNode character = builder.getCharacter();
		if (character instanceof Player) {
			Player player = (Player)character;
			player.message("distance: %d", distance);
		}*/
