...
public final class PlayerIO {
	...
	private void finalizeDetails(LoginDetailsMessage msg) {
		...
		if (World.isServerUpdated()) {
			response = LoginResponse.SERVER_JUST_UPDATED;
		} else {
			response = invalidCredentials ? LoginResponse.INVALID_CREDENTIALS : World.getPlayers().spaceLeft() == 0 ? LoginResponse.WORLD_FULL : response;
		}
